...
export default function Input({ className, label, error, id, ...props }) {
    return (
        <div className="flex flex-col gap 1.5">
            {label && (
                <label htmlFor={id} className="text-sm font-smitbold text-[var(--cream)]">
                    {label}
                </label>
            )}
            <input
                id={id}
                className={cn(
                    "h-11 w-full rounded-x1 border big-white/[0.05] px-4 text-sm text-[var(--cream)] placeholder:text-white/25 outline-none backdrop-blur-sm transition-all
                    duration 200",
...
export function Textarea({ className, label, error, id, ...props }) {
    return (
        <div className="flex flex-col gap-1.5">
            ...
            id={id}
            className={cn(
                "w-full rounded-x1 border bg-white/[0.05] px-4 py-3 text-sm text-[var(--cream) placeholder:text-white/25 outline-none backdrop-blur-sm trasition-all
